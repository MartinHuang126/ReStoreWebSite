...
Public Class Gather
    ...
    Public Sub ExcuteTask()
        ...
        CurrentUrlList.Add(Urlmain.Replace(":80", ""))
        saveAllFile()
    ...
    Public Sub DownloadFileForHtml(ByVal url As String, ByVal fullpath As String)
        ...
        End If
        strHtml = strHtml.Replace(UrlStartjs, "").Replace(UrlStartim, "").Replace(UrlStartoe, "").Replace(UrlStartcs, "") _
        .Replace(UrlStart, "").Replace(UrlEnd.TrimEnd("/"), "").Replace(UrlEnd.Replace("www.", ""), "").Replace("https://web.archive.org", "").Replace("http://www.10times10.com", "")
        ...
        Try
            If File.Exists(fullpath) Then
                LogHelp.writeLog(DateTime.Now.ToString & " jump a html file: " & fullpath & " URL: " & url & vbCrLf)
                writeToForm(DateTime.Now.ToString & " jump a html file: " & fullpath & " URL: " & url & vbCrLf)
                Return
            End If
            File.WriteAllText(fullpath, strHtml, System.Text.Encoding.UTF8)
        Catch ex As Exception
            LogHelp.writeLog(DateTime.Now.ToString & " writeFileError: " & fullpath & " , url: " & url & vbCrLf)
        End Try
        writeToForm(DateTime.Now.ToString & " done one html file: " & url & vbCrLf)
        LogHelp.writeLog(DateTime.Now.ToString & " done one html file: " & url & vbCrLf)
    End Sub
    ...
    Private Function getFilePath(Uri As String, staticFilepath As String, indexhttp As Integer, indexend As Integer, staticFileDirectory As String, staticFileName As String, phyimgDirectory As String) As String
        staticFilepath = Uri.Substring(indexhttp).Replace("10times10.com", SiteName)
    ...
    Private Function getIndexHttp(uri As String, index As Integer) As Integer
        If uri.Length > 30 Then
            index = uri.IndexOf(SiteName, 30)
            If index > -1 Then
                Return index
            End If
            Return uri.IndexOf("10times10.com", 30)
        End If
        Return -1
    End Function
    ...
    Private Sub GetUrlListPdf(ByRef strHtml As String)
        ...
        If Not ExisturlList.Contains(pdfUrl) Then
            NextUrlList.Add(pdfUrl)
            ExisturlList.Add(pdfUrl)
        End If
